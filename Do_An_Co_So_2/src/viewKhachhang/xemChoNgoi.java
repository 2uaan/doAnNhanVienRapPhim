...
public class xemChoNgoi extends JFrame {

	private JPanel panel_Khung;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					xemChoNgoi frame = new xemChoNgoi();
...
//		setLayout(new BorderLayout());
		...
		Background.setBounds(10, -34, 960, 447);
		...
		for (int i = 0; i<2; i++) {
			for (int j=0; j<8; j++) {
				...
				if (dem == 0) {
					...
					ganManHinh[i][j].setBackground(Color.red);
				}
		...
		for (int i = 0; i<2; i++) {
			for (int j=0; j<8; j++) {
				...
				if (dem == 0) {
					...
					vip[i][j].setBackground(Color.red);
				}
		...
		for (int i = 0; i<2; i++) {
			for (int j=0; j<4; j++) {
				...
				if (dem==0) {
					...
					Couple[i][j].setBackground(Color.red);
				}
